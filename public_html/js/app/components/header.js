...
function js(toolService, sessionService, $http, $location, countcarritoService) {
    ...
    $http({
        method: 'GET',
        url: 'http://localhost:8081/oncomic/json?ob=comic&op=getpage&rpp=10000&page=1'
    }).then(function (response) {
        self.status = response.status;
        self.ajaxDataProductosTodos = response.data.message;
    }, function (response) {
        self.status = response.status;
        self.ajaxDataProductosTodos = response.data.message || 'Request failed';
    });
