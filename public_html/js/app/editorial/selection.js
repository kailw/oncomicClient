...
function cController($http) {
    ...
    self.update = function (p) {
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/oncomicjson?ob=' + self.ob + '&op=getcount'
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/oncomicjson?ob=' + self.ob + '&op=getpage&rpp=' + self.rpp + '&page=' + self.page + self.orderURLServidor
        }).then(function (response) {
            self.status = response.status;
    ...
    self.resetOrder = function () {
        $http({
            method: "GET",
            withCredential: true,
            url: "http://localhost:8081/oncomic/json?ob=coleccion&op=getpage&rpp=10&page=1"
        }).then(function (response) {
            self.status = response.status;
            self.data = response.data.message;
        }, function (response) {
