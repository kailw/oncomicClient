...
moduleUsuario.controller('usuarioPlistFacturaController', ['$scope', 'toolService', '$http', 'sessionService', '$routeParams', '$location','pdfService',
    function ($scope, toolService, $http, sessionService, $routeParams, $location, pdfService) {
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/oncomic/json?ob=factura&op=getpagex&rpp=' + $scope.rpp + '&page=' + $scope.page + '&campo=id_usuario&id=' + $scope.id + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
